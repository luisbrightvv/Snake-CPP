...
bool isAxisAlignedStep(const sf::Vector2f& value) {
    const bool horizontal = std::fabs(value.x) == CELL_SIZE && value.y == 0.0f;
    const bool vertical = std::fabs(value.y) == CELL_SIZE && value.x == 0.0f;
    return horizontal || vertical;
}

sf::RectangleShape makeSegment(const sf::Vector2f& position) {
    sf::RectangleShape segment;
    segment.setSize(sf::Vector2f(CELL_SIZE, CELL_SIZE));
